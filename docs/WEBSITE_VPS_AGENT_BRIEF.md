...
|---------|-----------|----------|------|----------|
| Next.js frontend | `website/frontend/` | `/var/www/website/website/frontend` | 3000 | `oriasc-web` |
| Strapi backend | `website/backend/` | `/var/www/website/website/backend` | 1337 | `oriasc-strapi` |
...
Reorganize the monorepo if needed:
...
**Agent tasks:**
1. Move Next.js app → `website/frontend/` (use `git mv` to preserve history).
2. Move Strapi app → `website/backend/`.
3. Fix all relative paths, CI, and import paths after the move.
4. Add root `.gitignore` entries for secrets and build artifacts.
5. Add `deploy/` folder with files listed below.
...
### 4.2 `website/backend/.env` (Strapi — gitignored)
...
### 4.3 `website/frontend/.env.production` (Next.js — gitignored)
...
module.exports = {
  apps: [
    {
      name: "oriasc-strapi",
      cwd: "/var/www/website/website/backend",
      script: "npm",
      args: "run start",
      instances: 1,
      autorestart: true,
    ...
    {
      name: "oriasc-web",
      cwd: "/var/www/website/website/frontend",
      script: "npm",
      args: "run start",
      instances: 1,
      autorestart: true,
...
docker compose --env-file deploy/env/postgres.env -f deploy/docker-compose.yml up -d
cd website/backend
test -f .env || { echo "Missing website/backend/.env"; exit 1; }
chmod 600 .env
npm ci
npm run build
cd "$APP_DIR"
...
cp deploy/env/strapi.env.example website/backend/.env
cp deploy/env/frontend.env.example website/frontend/.env.production
...
```bash
# Frontend only
bash /var/www/website/deploy/scripts/deploy-frontend.sh main

# Strapi / content model changes
bash /var/www/website/deploy/scripts/deploy-backend.sh main

# Both
bash /var/www/website/deploy/scripts/deploy-all.sh main
```
...
- [ ] Persistent volume for Strapi `public/uploads`
...
1. Repo restructured to `website/frontend` + `website/backend`
2. Complete `deploy/` folder (compose, nginx, pm2, scripts, env examples)
3. `DEPLOYMENT.md` with step-by-step VPS instructions
4. `.env.example` files for both apps (no secrets committed)
5. `next.config` configured for production domain (`oriasc.org`)
6. Strapi `config/server.ts` / `config/middlewares.ts` with correct `url` and CORS
7. Verified build locally: `npm run build` in both folders
8. Documented PM2 process names and health-check URLs
9. No changes to `/var/www/hrms` or HRMS deployment files
...
**Mirror the same patterns** for the website repo; keep stacks independent.
